...
class ProfileController extends Controller
{
    ...
    public function progress(Request $request)
    {
        ...
        if ($user && $user->role === 'parent') {
            ...
            // Only show year filters for the parent's children
            $years = $user->children()->with('class')->get()->pluck('class.year')->unique()->filter()->values()->all();
        } else {
            $years = ['4', '5', '6'];
        }
        ...
        $progress = \App\Models\Progress::whereIn('child_id', $children->pluck('id'))
            ...
            ->keyBy('child_id')
            ->toArray();
        return view('progress.index', compact('children', 'years', 'selectedYear', 'selectedDate', 'progress', 'classrooms'));
    }
